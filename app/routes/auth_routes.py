...
def issue_dev_token(payload: DevTokenRequest, settings: Settings = Depends(get_settings), db: Session = Depends(get_db)) -> DevTokenResponse:
    if not settings.allow_dev_tokens:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="dev token issuance disabled")

    try:
        token = auth.issue_dev_token(db, settings, user_id=payload.user_id, tenant_id=payload.tenant_id, aud=payload.aud)
